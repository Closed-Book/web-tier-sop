...
Codex 适配只做安装路径适配：把同一套 `web-tier` / `opencli-web` skills
软链到 `~/.agents/skills/`。它不改变 web-tier 原有 SOP，也不新增 Codex 专用路由策略。
...
Codex 或双宿主用户可以显式选择安装目标：

```bash
./install.sh --host codex    # Codex 用户
./install.sh --host both     # 同时使用 Claude Code 和 Codex
```
...
Codex 用户运行 `./install.sh --host codex` 即可复用同一套
`web-tier` / `opencli-web` skills。这里仅适配安装目录，不提供额外路由模板。
