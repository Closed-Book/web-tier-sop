...
**Claude Code / Codex 的 Web 任务路由 SOP**

让 AI 在每个联网任务前先做路由决策，而不是无脑试 `WebFetch` / `WebSearch`。

`macOS` · `Claude Code` · `Codex` · `Node.js 22+` · `MIT`
...
Codex 不直接把 `web-tier` 当默认浏览器自动化入口。安装到 Codex 时，会额外提供
[`codex-web-routing`](skills/codex-web-routing/SKILL.md)：

| Codex 场景 | 首选 |
|---|---|
| localhost / 本地前端 / file preview / 公开网页 | Codex in-app Browser |
| 登录态页面 / 已有 Chrome 会话 / cookies / 扩展 | Codex Chrome extension |
| GUI 复杂操作 / 跨 App / file picker / 系统权限弹窗 | Computer Use |
| 结构化、批量、定时、远程、高风控、独立 profile | `web-tier` + `opencli-web` |

也就是说：Codex 原生能力优先，`web-tier` 作为稳定后备和结构化抓取链路。
...
├── skills/
│   ├── codex-web-routing/           # Codex 专用入口分流 skill
...
└── docs/
    ...
    └── codex-agents-routing.md      # 可选：加进 AGENTS.md 的 Codex 路由片段
```
...
```bash
./install.sh --host claude   # 默认：只安装 Claude Code 入口
./install.sh --host codex    # 只安装 Codex 入口
./install.sh --host both     # Claude Code + Codex 都安装
```

`install.sh` 会：按宿主软链 skill 到 `~/.claude/skills/` 或 `~/.agents/skills/`、
`npm ci` 装 opencli 依赖、拷 runtime 脚本到 `~/.web-tier/`、生成
`~/.web-tier/bin/web-tier-helper` / `~/.web-tier/bin/opencli-web` wrapper，并把 plist 模板渲染到
...
Codex 用户主要使用 `codex-web-routing` skill；如需规则常驻，也可把
[`docs/codex-agents-routing.md`](docs/codex-agents-routing.md) 的片段复制进全局
`~/.codex/AGENTS.md` 或项目 `AGENTS.md`。
...
ls -l ~/.web-tier/bin/web-tier-helper
~/.web-tier/bin/web-tier-helper version
```

如果独立 Chrome 尚未预热，`version` 可能失败；先按上面的预热流程启动 9223 后再测。
