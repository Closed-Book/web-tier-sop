...
~/.web-tier/bin/web-tier-helper wait "$TID" 15000
~/.web-tier/bin/web-tier-helper check-login "$TID" x
...
```bash
~/.web-tier/bin/web-tier-helper alert "X 登录态失效，AI 日报跳过 X 板块"
# 跳过 X 抓取
```
...
TID=$(~/.web-tier/bin/web-tier-helper new "https://x.com/login")
...
- 看 tab：`~/.web-tier/bin/web-tier-helper list`
