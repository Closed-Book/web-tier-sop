...
在 Codex 下，本 skill 不是默认入口；先使用 `codex-web-routing` 判断 Codex Browser、Chrome extension、Computer Use 是否足够。

只有命中结构化、批量、定时、远程、高风控、独立 profile、或 Codex 原生浏览器链路失败等场景时，才把本 skill 当重型后备链路调用。Codex 文档与 skill 中优先使用安装脚本生成的 wrapper：

```bash
~/.web-tier/bin/web-tier-helper version
~/.web-tier/bin/opencli-web doctor
```

Claude Code 原有入口和下面三层路由规则保持不变。
...
```bash
~/.web-tier/bin/web-tier-helper <command> [args]
```

Claude Code 旧安装路径 `node ~/.claude/skills/web-tier/helper.mjs <command> [args]` 仍可用；新文档优先使用 wrapper，避免 Codex 依赖 Claude 专属目录。
...
```bash
~/.web-tier/bin/web-tier-helper refresh-cookies-from-main
...
~/.web-tier/bin/web-tier-helper list                        # 看当前 tab
```
